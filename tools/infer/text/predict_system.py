...
class TextSystem(object):
    ...
    def __call__(self, img_or_path: Union[str, np.ndarray], do_visualize=False):
        """
        ...
        if True:
            vst = time()
            vis_fp = os.path.join(self.vis_dir, "text_ocr_res.png")
...
def predict_system(image_path, save_dir):
    # parse args
    args = parse_args()
    ...
    for i, img_path in enumerate(img_paths):
        ...
        boxes, text_scores, time_prof = text_spot(img_path, do_visualize=False)
