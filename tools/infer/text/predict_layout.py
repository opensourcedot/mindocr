...
def save_layout_res(layout_res: List, img_path: List[str], save_dir: str):
    lines = []
    img_name = os.path.basename(img_path).rsplit(".", 1)[0]
...
if __name__ == "__main__":
    ...
    for i, img_path in enumerate(img_paths):
        ...
        assert os.path.exists(img_path) == True, f"image_path {img_path} not exists"
        image = cv2.imread(img_path)
        assert image is not None, f"layout input image is not a valid image"
